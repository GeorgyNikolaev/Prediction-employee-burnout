...
class WordEncoder(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_heads=4):
        ...
        self.keys = nn.Parameter(torch.Tensor(embedding_dim, embedding_dim))
        self.keys.data.uniform_(-1.0, 1.0)
        self.queries = nn.Parameter(torch.Tensor(embedding_dim, embedding_dim))
        self.queries.data.uniform_(-1.0, 1.0)
        self.values = nn.Parameter(torch.Tensor(embedding_dim, embedding_dim))
